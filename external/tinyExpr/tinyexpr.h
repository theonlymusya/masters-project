...
template <typename T>
constexpr bool te_is_function_v = !te_is_constant_v<T> && !te_is_variable_v<T> && !te_is_closure_v<T>;
...
using te_variant_type = std::variant<te_type,
                                     const te_type*,  // indices 0-1
                                     te_fun0,
                                     te_fun1,
                                     te_fun2,
                                     te_fun3,
   ...
   private:
    ...
    void validate_name(const te_variable& var) const {
        ...
        if (!is_letter(var.m_name[0]) && var.m_name[0] != '_') {
            throw std::runtime_error(std::string("Variable name must begin with a letter from a-z or _: ") +
                                     var.m_name);
        }
        ...
        if (varCharPos != var.m_name.cend()) {
            throw std::runtime_error(std::string("Invalid character in variable name: ") + var.m_name);
        }
    }
...
#undef TE_DEF_CLOSURE
    ...
    [[nodiscard]]
    inline static te_expr* new_expr(const te_variable_flags type,
                                    te_variant_type value,
                                    const std::initializer_list<te_expr*>& parameters) {
        te_expr* ret = new te_expr{type, std::move(value)};
        ret->m_parameters.resize(std::max<size_t>(
            std::max<size_t>(parameters.size(), get_arity(ret->m_value)) + (is_closure(ret->m_value) ? 1 : 0),
            0));
        if (parameters.size()) {
