...
static te_type te_round(te_type val, te_type decimalPlaces)  // NOLINT
{
    ...
    const size_t adjustedDecimalPlaces{
        !std::isfinite(decimalPlaces) ? 0 : static_cast<size_t>(std::abs(decimalPlaces))};
    ...
    // ROUND(21.5, -1) = 20
    if (val < 0) {
        return std::ceil(static_cast<te_type>(val / decimalPostition) - ROUND_EPSILON) * decimalPostition;
    }
...
[[nodiscard]]
static te_type te_asset_depreciation(te_type cost,
                                     te_type salvage,
                                     te_type life,
                                     te_type period,
                                     te_type month) {
...
[[nodiscard]]
static te_type te_sum(te_type val1,
                      te_type val2,
                      te_type val3,
                      te_type val4,
                      te_type val5,
    ...
    return getSumMaybeNan(val1) + getSumMaybeNan(val2) + getSumMaybeNan(val3) + getSumMaybeNan(val4) +
           getSumMaybeNan(val5) + getSumMaybeNan(val6) + getSumMaybeNan(val7) + getSumMaybeNan(val8) +
           getSumMaybeNan(val9) + getSumMaybeNan(val10) + getSumMaybeNan(val11) + getSumMaybeNan(val12) +
...
[[nodiscard]]
static te_type te_average(te_type val1,
                          te_type val2,
                          te_type val3,
                          te_type val4,
                          te_type val5,
    ...
    const auto validN =
        isValidMaybeNan(val1) + isValidMaybeNan(val2) + isValidMaybeNan(val3) + isValidMaybeNan(val4) +
        isValidMaybeNan(val5) + isValidMaybeNan(val6) + isValidMaybeNan(val7) + isValidMaybeNan(val8) +
        isValidMaybeNan(val9) + isValidMaybeNan(val10) + isValidMaybeNan(val11) + isValidMaybeNan(val12) +
        isValidMaybeNan(val13) + isValidMaybeNan(val14) + isValidMaybeNan(val15) + isValidMaybeNan(val16) +
        isValidMaybeNan(val17) + isValidMaybeNan(val18) + isValidMaybeNan(val19) + isValidMaybeNan(val20) +
        isValidMaybeNan(val21) + isValidMaybeNan(val22) + isValidMaybeNan(val23) + isValidMaybeNan(val24);
    const auto total =
        te_sum(val1, val2, val3, val4, val5, val6, val7, val8, val9, val10, val11, val12, val13, val14, val15,
...
[[nodiscard]]
static te_type te_max(te_type val1,
                      te_type val2,
                      te_type val3,
                      te_type val4,
                      te_type val5,
...
[[nodiscard]]
static te_type te_min(te_type val1,
                      te_type val2,
                      te_type val3,
                      te_type val4,
                      te_type val5,
...
static te_type te_and_maybe_nan(te_type val1, te_type val2MaybeNan) {
    return !std::isfinite(val2MaybeNan) ? static_cast<te_type>(te_parser::number_to_bool(val1))
                                        : static_cast<te_type>(te_parser::number_to_bool(val1) &&
                                                               te_parser::number_to_bool(val2MaybeNan));
}

[[nodiscard]]
static te_type te_and_variadic(te_type val1,
                               te_type val2,
                               te_type val3,
                               te_type val4,
                               te_type val5,
...
[[nodiscard]]
static te_type te_or_variadic(te_type val1,
                              te_type val2,
                              te_type val3,
                              te_type val4,
                              te_type val5,
...
[[nodiscard]]
static te_type te_ifs(te_type if1,
                      te_type if1True,
                      te_type if2,
                      te_type if2True,
                      te_type if3,
...
const std::set<te_variable> te_parser::m_functions = {  // NOLINT
    ...
    {"clamp",
     static_cast<te_fun3>([](const te_type num, const te_type start, const te_type end)  // NOLINT
                          {
    ...
    do  // NOLINT
    {
        ...
        if (((*theState->m_next >= '0') && (*theState->m_next <= '9')) ||
            ...
            if (is_letter(*theState->m_next) || (*theState->m_next == '_')) {
                ...
                else if (tok == '<' && (*theState->m_next == '<') && (*std::next(theState->m_next) == '<')) {
                    theState->m_type = te_parser::state::token_type::TOK_INFIX;
...
te_expr* te_parser::power(te_parser::state* theState) {
    ...
    while (theState->m_type == te_parser::state::token_type::TOK_INFIX &&
           ((is_function2(theState->m_value) && (get_function2(theState->m_value) == te_builtins::te_add ||
                                                 get_function2(theState->m_value) == te_builtins::te_sub))
#ifndef TE_FLOAT
        ...
        if (is_function2(theState->m_value) && get_function2(theState->m_value) == te_builtins::te_sub) {
            theSign = -theSign;
    ...
    return std::visit(
        [&, texp](const auto& var) -> te_type {
            ...
            if constexpr (te_is_constant_v<T>) {
                ...
                return std::apply(var, make_closure_arg_list(M, texp->m_parameters[n_args - 1],
                                                             std::make_index_sequence<n_args - 1>{}));
            } else if constexpr (te_is_function_v<T>) {
                constexpr size_t n_args = te_function_arity<T>;
                return std::apply(var, make_function_arg_list(M, std::make_index_sequence<n_args>{}));
            } else {
                return te_nan;
            }
        },
...
std::string te_parser::info() {
    ...
    sysInfo += "Max supported integer:    " + std::to_string(static_cast<uint64_t>(get_max_integer())) + "\n";
